...
void organiser::addFragment(wQueue<message>* qList, int N) {
    ...
    while (count < N) {
        ...
        qList->enqueue(msg, msg.getPriority());
        ++count;
    ...
    while (processedFragments < initialQueueSize) {
        ...
        if (currentMessage.getFrag() == expectedFrag) {
            ...
            if (currentMessage.getReenqueueCount() <= MAX_REENQUEUE_COUNT) {
                qList->enqueue(currentMessage,currentMessage.getPriority());  // Re-enqueue the fragment
            }
            /*else {
...
int organiser::calculatePriority(int fragNumber) const {
    if (fragNumber <= 5) return 1;  // High priority
    if (fragNumber <= 10) return 2; // Medium priority
    return 3;                       // Low priority
}
...
message organiser::getNextFragment() {
    int convID, fragNumber, lengthMsg;

    if (inputFile.eof()) {
        throw std::runtime_error("No more fragments to read.");
    }

    // Read fragment metadata
    if (!(inputFile >> convID >> fragNumber >> lengthMsg)) {
        throw std::runtime_error("Error reading fragment data.");
    }
    ...
    if (inputFile.gcount() != lengthMsg) {
        throw std::runtime_error("Incomplete fragment read.");
    }

    // Calculate and set the message data
    int priority = calculatePriority(fragNumber);
    message msg;
